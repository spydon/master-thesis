...
\begin{equation} \label{eq:fileread}
    \inferrule
    {A[u,c] \\ C[c \cup \{m\}]} 
    {(C[c], F) \xrightarrow{u, read(f)} (C[c], F)}
\end{equation}
\vspace{1em}
...
\begin{equation} \label{eq:snapshot}
    \begin{split}
        &snapshot(c) = \{c'.id = \textit{"fresh"}, snapshot(c') \mid c' \in c \} \cup \{ snapshot(m) \mid m \in c \}\\
...
\begin{equation} \label{eq:snapshot2}
    \inferrule
    {A[u,c] \\ \neg readOnly(c)}
    {(C[c], F) \xrightarrow{u, copy(c)} (C[c \cup snapshot(c)], F)}
\end{equation}
