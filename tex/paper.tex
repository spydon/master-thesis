...
\fix Write about Bell-Lapadula et. al
...
systems: A mathematical model~\cite{BLP2}. In these papers the foundation is laid for how to model
...
strong idea of the soundness of the designed system.
...
\begin{center}
    \begin{tabular}{ | l | l | l | p{5cm} |}
        ...
        u, modify(o)          & User u globally modifies object o\\ \hline
...
\subsubsection{Consequences of modification}
...
\par
As the project tree is built by querying documents parent id one optimisation that could be done was
to create an index of the documents parent ids. This drastically improved the size of the tree that
the server was able to handle within the time out, going from being able to handle 4000 documents to
at least 50000 documents, which the browser instead will have problems displaying, represented in the 
UI, without lag.
...
The authorization of users in the system is currently being handled by LDAP~\cite{LDAP}, as Perius is mainly
focussed at being deployed in internal networks which usually has an LDAP service enabled. LDAP also
