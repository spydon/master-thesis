...
handle it very well as one file can not be shown or uploaded in two different places in the virtual
filesystem.
...
\subsubsection{Run-time of Operations}
...
trees balance is not determined by an algorithm but rather by a user, but from observation one 
could argue that the trees usually have several nodes for each parent node in the tree, which then
would give a logarithmic tight bound, $\theta(log(n))$.\\

\par \textbf{POST entity} \\
