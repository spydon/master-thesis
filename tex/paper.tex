...
implementation was made. It also gives some brief explanation of the core concepts which are 
...
\subsubsection{Terms}
\subsubsubsection{Eventual Consistency}
...
solve this the system needs to have defined which operation that will take precedence, or how the
concurrent operations are mixed, and this is usually called conflict or sibling resolution.

\subsubsubsection{Perius}
...
The model for this work should show how the data can only be accessed or modified by authorized
...
basically just meta data and not classed as important, from a data integrity point of view, as the 
...
and meta-data can only be changed within a project and the system can contain several projects 
...
transactions were used and the threads were running fully concurrently and without any sleep
statements. 
...
handle it very well as one file can not be shown or uploaded in two different places in Battle
...
are needed and does not scale very well, a new system should be developed. This work is about
...
React can be seen as the system for handling views in front-ends implementing a MVC
...
\subsubsection{Scala} 
Scala is a multi-paradigm programming language. It most commonly runs on the JVM and as apart from
...
the happened-before relation. In this work this problem is solved by implementing Copy-on-Write on
data with high consistency requirements.
...
refers to the same binary files until the meta-data is changed to point to another file.
...
of meta-data that were in the left tree, for example C5 and C2 contain the same data (except for
their newly generated IDs). The green nodes refer to files that have not been copied but are still
referenced in the copied meta-data, this is due to the fact that a snapshot is not a write to the
...
The service features a virtual file structure over the assets and meta-data that have been stored,
...
\subsubsection{Run-time Complexity of Operations} \label{sec:run_time}
...
Figure~\ref{fig:storage}, however in reality it not quite so table-like. One important point that 
...
REST was chosen as all operations can be defined as basic CRUD operations needs to be performed and
because the BSON format which is used in MongoDB is almost identical ~\cite{BSON} to the
...
constantly have to, in some sense, be fully aware of the state of the other nodes. Sharding could be
used to minimise the communication of state between the nodes, but that results in less fault
tolerance and possibly higher loads on some nodes than others due to that the data most likely wont
...
building the project trees is (as mentioned in Section~\ref{sec:load_testing}) solved.
