...
authorized users and how the defined integrity of the data is always kept in the Perius system. 
...
\begin{equation} \label{eq:snapshot}
    \begin{split}
        [C', M'] = u, snapshot(tree) \Implies \\
        \forall c \in tree, u, copy(c) \in C \land \\
        \forall m \in tree, u, copy(m) \in M \land \\
        (\forall f \in tree \Implies f \in F)
    \end{split}
\end{equation}
...
\begin{equation} \label{eq:snapshot2}
    \inferrule
    {c \in C \\ A[u,c] \in A}
    {M \land C \xrightarrow{[C', M'] = u, snapshot(c)} M \cup M' \land C \cup C'}
\end{equation}
