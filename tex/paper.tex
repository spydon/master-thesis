...
\begin{equation} \label{eq:predicate_rules}
    \inferrule
    {(\textbf{*}) \\ C \ni c \ni m \\ A[u,c]} 
    {}
\end{equation}
...
\begin{equation} \label{eq:copy}
    ...
    {(C[c \cup \{m\}], F) \xrightarrow{u, create(c, m')} (C[c \cup \{m, m'\}], F)}
\end{equation}
\vspace{1em}
...
\begin{equation} \label{eq:delete}
    \inferrule
    {\textbf{*} \\ \neg readOnly(c)}
    {(C[m], F) \xrightarrow{u, delete(m)} (C[], F)}
\end{equation}
\vspace{1em}
...
\begin{equation} \label{eq:snapshot2}
    \inferrule
    {\textbf{*} \\ \neg readOnly(c)}
    {(C, F) \xrightarrow{[C', M'] := u, snapshot(c)} (C[C' \cup M'], F)}
\end{equation}
