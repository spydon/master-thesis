...
and then directly available to the public.  For secret content this can be a problem and that is
what this thesis is about. This work examines ways of enforcing access control on content and groups
of content in the form of folders and snapshots. A system was developed to make the underlying
theory work in practice. 

The research question that this report answers is whether it is practically feasible to use
Copy-on-Write for a high-level system like the one that is implemented. The work also thoroughly
compares different types of lazy snapshots to figure out which is the most

\subsection{Prior Work}
\subsubsection{Copy-on-Write}
This work relies heavily on the Copy-on-Write principle which was founded and used in the Mach
kernel~\cite{COPYONWRITE} as it can be used to efficiently create snapshots. 

Copy-on-Write is used in virtual memory management systems~\cite{VIRTCOW}, for snapshot
functionality in both file systems~\cite{BTRFS} and logical volume management systems~\cite{LVMCOW},
and as an optimisation technique for objects and types in programming languages\cite{LANGCOW}.
...
pointers to the data. After many Copy-on-Write's a complex structure can be built up, but it is
possible to solve that structure~\cite{COPYONWRITE2}.
...
experiences, that means that they are not focussed on the actual gameplay which other EA studios
like DICE is. 
...
\section{Model}
The model for this work should make sure that data can not be accessed by unauthorized users in the
Perius system (See section TODO: fill in). There could also be another model done to make sure that
content can not be accessed by unauthorized viewers once the content is uploaded to the CDN, but as
that should already have been thoroughly checked by the CDN providers this work can focus on the
internal users of the management system. 
...
In the mid 80s when the development of the Mach kernel started, there was problems with that
