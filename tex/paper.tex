...
\subsection{Architecture and Technologies used}
...
\begin{description}
\item[React]% \hfill \\ % \label{sec:react}
...
% \hfill \\ % \label{sec:reflux}
\item[Reflux]~\cite{REFLUX} 
is an idea and a simple library of how to structure your application. It features a unidirectional
dataflow (see Figure~\ref{fig:reflux}) which makes it more suitable, than for example
...
\item[Scala]% \hfill \\ %\label{sec:scala}
...
\item[REST]% \hfill \\ %\label{sec:rest}
...
projects and nodes in the tree structures and also the files uploaded to the system. Scala uses a
layer called Casbah~\cite{CASBAH} to interface with MongoDB.
